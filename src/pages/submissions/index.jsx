...
export default function SubmissionsPage() {
  ...
  const { submissions, loading, error, updateLoading } = useSelector(
  ...
  const handleStatusUpdate = async (submissionId, status) => {
    ...
    if (
      status === "approved" &&
      (!actionData.score || actionData.score < 0 || actionData.score > 10)
    ) {
    ...
    if (
      status === "rejected" &&
      (!actionData.remarks || actionData.remarks.trim() === "")
    ) {
  ...
  const getActionState = (submissionId) => {
    return (
      actionStates[submissionId] || { action: null, score: "", remarks: "" }
    );
  };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
              Review and manage pending submissions from learners across all
              levels
            </p>
          </div>
                  ...
                  <div className="inline-flex items-center justify-center w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-full mb-4">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.29-1.009-5.824-2.412M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"
                      />
                    </svg>
                  </div>
                  <p className="text-lg font-medium text-slate-600 dark:text-slate-300">
                  ...
                  <div className="p-6 space-y-6">
                    {submissions.map((submission) => {
                      ...
                      return (
                        <div
                          key={submission._id}
                          className="group bg-slate-50 dark:bg-slate-700/50 rounded-xl p-6 border border-slate-200 dark:border-slate-600 hover:shadow-lg hover:border-emerald-300 dark:hover:border-emerald-600 transition-all duration-200"
                        >
                          {/* Header Info */}
                          <div className="mb-6">
