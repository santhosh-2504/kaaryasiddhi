...
export default function PaymentsPage() {
  ...
  const getPaymentTypeColor = (type) => {
    const colors = {
      membership: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-lg backdrop-blur-sm border border-slate-200 dark:border-slate-700 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-slate-900 dark:text-slate-100">{payments.length}</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-lg backdrop-blur-sm border border-slate-200 dark:border-slate-700 p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Avg. Amount</p>
                  <p className="text-3xl font-bold text-slate-900 dark:text-slate-100">
                    {payments.length > 0 ? formatCurrency(payments.reduce((sum, payment) => sum + payment.amount, 0) / payments.length) : '₹0'}
                  </p>
                </div>
          ...
          {loading ? (
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-xl backdrop-blur-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
              ...
              <p className="text-lg font-medium text-slate-600 dark:text-slate-300">Loading payments...</p>
            </div>
          ) : payments.length === 0 ? (
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-xl backdrop-blur-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">All payments have been processed</p>
            </div>
          ) : (
                ...
                <table className="w-full">
                  <thead className="bg-slate-50 dark:bg-slate-700/50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Month</th>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Paid Date</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Screenshot</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                    {payments.map((payment) => (
                      <tr key={payment._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-200">
                ...
                {payments.map((payment) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500 dark:text-slate-400">{payment.month}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Paid Date</p>
